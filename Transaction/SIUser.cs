...
namespace POS.Transaction
{
    public class SIUser : IDataCRUD
    {
        ...
        private static IConnection connection = new Connection.Connection();
        #endregion
        ...
        public SIUser()
        {
            ...
            var command =
                new SqlCommand(
                    "SELECT USER_CODE,USER_NAME,USER_DESC,USER_CPAS,USER_STAT,USER_LOG FROM SIPUSERS ORDER BY USER_CODE",
                    connection.Connect());
        ...
        public void  ShowListView(ListView lv)
        {
            try
            {
                ...
                {
                   var concate =
                        "SELECT USER_CODE,USER_NAME,USER_DESC,USER_CPAS,USER_STAT,USER_LOG FROM SIPUSERS WHERE DB_CODE IN (SELECT GR_DB_CODE FROM dbo.SIUSERSGP WHERE PER_TYPE = 'D' AND USER_CODE='" +
                        Code +
                        "') UNION SELECT DISTINCT USER_CODE,USER_NAME,USER_DESC,USER_CPAS,USER_STAT,USER_LOG FROM SIPUSERS WHERE USER_CODE IN (SELECT USER_CODE FROM SIPUSERP WHERE GR_DB_CODE IN (SELECT GR_DB_CODE FROM dbo.SIPUSERP WHERE PER_TYPE = 'D' AND USER_CODE='" +
